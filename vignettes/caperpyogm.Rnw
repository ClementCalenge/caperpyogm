...
\item In section \ref{sec:other-models-detect}, we demonstrate how a
  ...
  detection process during lek censuses.\\
...
Let $y_{i,t,k}$ be the number of males counted during the census $k$ of the year $t$ on the lek $i$. We model these counts
...
is the probability of detection characterizing this census on
...
\item The two-years period during which  the census occurred
...
Here, each point corresponds to a census.  These residuals do
...
Note that a group of censuses with a residual greater than 6
...
and censused only this year. We study more in details in section
...
model. One possibility is to calculate, for each census, the
...
We can also, for each census, calculate a 80\% credible
interval on the expected detected number of males (using the simulated
datasets), and calculate the proportion of censuses falling in
...
model using the coefficient $\tau$ of \cite{Garrett2000}, measuring
the overlap between the prior and the posterior distribution. Let $x$
be a parameter of the model, $P(x)$ is the prior distribution for this
parameter, and $P(x|\mathbf{D})$ is the posterior distribution. Then,
the parameter $\tau$ measures the overlap:
$$
...
\texttt{caperpyogm} (see the help page of this functions) to calculate
the coefficient $\tau$ for the variances of random effects in the
models, and the function \texttt{plotOverlap} from the same package to
...
area of potential presence of the species (defined by experts in 2009,
before the onset of the program), and $h_q$ is a binary variable
taking the value 1 if the grid cell $q$ contains a known lek and 0
otherwise. The intercept $a_{g(q)}$ and slope of the area of potential
presence $b_{g(q)}$ is
...
\item The proportion of the grid cell covered by the area of potential
  presence of the capercaillie (as defined by a group of experts in
...
this model, using the parameter $\tau$ of \cite{Garrett2000} (see
...
fact that we should not model the detection process during prospection
...
\begin{itemize}
\item proportion of the cell covered by the potential presence area of the
  capercaillie
...
out $n$ censuses of the male capercaillie on this lek. We suppose, in
our example, that the number of detected males $y_i$ during census $i$
is drawn from a binomial distribution with parameters $N$ (the actual
number of males) and $d$ (the detection probability). We measure the
quality of our prediction by our model by calculating the elpd on four
model, i.e. by trying to predict the number of detected
males in $n$ new censuses in the same conditions.\\
...
males during the census $i$ on the lek in our new dataset).
...
sample, we include \textit{all} the censuses carried out on a leks (all
censuses and all years) to the same subsample. This approach is
...
years, with one or several censuses every year), and these $N$
...
the number of censuses differs a lot between leks (between 1 to
19 censuses in 10 years). But the probability to observe a
given set of censuses will decrease when the number of censuses
increase: for example, if $p$ is the probability to detect $n$ males
during a census, then the probability to detect $n$ males
during each one of two censuses is $p^2$ if these two censuses
are independent. In other words, the Therefore, the probability to
observe a given set of censuses will decrease with the size of the
set. In other words, the logarithm of the mean of 
$p(\mathbf{y}_i|\theta^{k,s}, \mathcal{M}_0)$ over all simulations $s$
will be much smaller for leks with many censuses: these leks
...
However, note that in our study, the $c_i$ censuses carried out on a
given lek \textit{are not} independent: some censuses are carried out
...
$p(\mathbf{y}_i| \mathbf{N}_i^{(r)}, \theta^{k,s})$
to observe the set of detected males during all censuses for a
...
animals (the two censuses carried out on this lek resulted in 18 and 17
males respectively).\\
...
and known leks for censuses by natural unit. First, a sample of natural
...
known, and only a small number of them is censused every year (though
...
We used a map of the area of potential presence of the species defined
...
by the area of potential presence. Moreover, we decided to no longer
stratify the grid cells by natural units: it appeared that there were
no clear differences of the probability of presence of UL between the
