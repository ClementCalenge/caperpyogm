simulateDataList <- function (coefs, dataList, betaBinDelta=NULL, doubleCountsp=NULL)
{
    ...
    sigmaeta <- apply(1/sqrt(MCMCchains(coefs, params = "sigmaeta")),2,median)
    ...
    for (i in 1:dataList$Nlekperiods) {
        ...
        for (k in 1:dataList$repetition[i]) {
            lpd[i, k] <- interceptpd + pente1pd * dataList$observers[i, k] +
                REYt[dataList$gr[dataList$lek[i]],
                  dataList$year[i, k]]
