...
al (in prep.).The aim of this paper is to estimate the number of male
...
\begin{equation}
  \label{eq:ug}
\widehat{S_{g3}} = \sum_{q=1}^{Q_g} \hat{\pi}_q
\end{equation}
...
males on KILs, we could benefit from a larger dataset. Moreover, the
